...
const state = {
  ...
  activeValgstedId: "85103", // Løvvanghallen som default
  activeValgstedNavn: "Løvvanghallen", // Default valgstedsnavn
  visningsUrl: "../views/valgsteder-resultater.html", // Bemærk ændret sti til visningen
...
function initialize() {
  document.addEventListener("DOMContentLoaded", () => {
    ...
    // Set active municipality and polling stations
    ...
    // Update preview - using the correct URL structure
    updatePreviewUrl();
    ...
    log(
      `Aktiv kommune: ${state.activeKommuneId}, valgsted: ${state.activeValgstedId}`
    );
  });
}
...
function initializeValgstedButtons() {
  ...
  log(
    `Initialiserer valgstedknapper: Fandt ${kommuneValgsteder.length} valgsteder for kommune ${state.activeKommuneId}`
  );
...
function setAktivKommune(id) {
  ...
  if (kommuneValgsteder.length > 0) {
    ...
    // Still update preview with empty valgsted
    updatePreviewUrl();
  }
...
async function fetchAndUpdateData() {
  try {
    ...
    if (data.result) {
      ...
      const votesGiven = data.result.votesGiven || 0;
      el("antalStemmer").textContent = `${formatNumber(votesGiven)} stemmer`;
    }
