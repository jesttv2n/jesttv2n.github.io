/**
 * Controller for the election results control panel
 */
...
function initialize() {
  document.addEventListener("DOMContentLoaded", () => {
    ...
    // Log system start
    log("Valgresultater-kontrolpanel initialiseret");
  });
}
...
async function fetchAndUpdateData() {
  try {
    log(`Henter valgdata for kommune ${state.activeKommuneId}...`);
    ...
    if (data.result) {
      ...
      if (data.result.votesPercentage) {
        el(
          "valgdeltagelse"
        ).textContent = `${data.result.votesPercentage.toFixed(1)}%`;
      } else {
    ...
    log(`Data opdateret for ${data.result?.name || "kommune"}`);
    return data;
  } catch (error) {
...
function sendToCasparCG() {
  ...
  // Log command
  log(
    `Sender til CasparCG (${host}:${port}): Viser ${kommune.navn} valgresultater på kanal ${channel}`
  );
  ...
  setTimeout(() => {
    log(
      `CasparCG bekræftede: Viser ${kommune.navn} valgresultater på kanal ${channel}`
    );
  }, 500);
}
